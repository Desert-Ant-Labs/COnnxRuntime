...
// name/run/read surface desert-ant-core's `CLiteRt` does, so its `OnnxSession`
// is the same shape as its `LiteRTSession`. ORT's lifecycle (api table,
// environment, session options, execution provider selection, OrtValue
// construction, status checking) stays here in C.
//
